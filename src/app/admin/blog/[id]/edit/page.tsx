...
import { updateBlogPost, getAdminBlogPostById } from '@/lib/supabase/blog-actions'
import { ArrowLeft, CheckCircle, Upload, Loader2, ImageIcon, X, RefreshCw, AlertCircle } from 'lucide-react'
...
export default function EditBlogPostPage({ params }: { params: { id: string } }) {
    ...
    async function fetchPost() {
      ...
      if (result.success && result.data) {
        ...
        setFormData({
          ...
          relatedProducts: (post.related_products || []).join(', '),
          tags: (post.tags || []).join(', '),
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await updateBlogPost(params.id, fd)
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-3 text-warm font-sans">Loading blog post...</span>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <Link
          href="/admin/blog"
          className="p-2 hover:bg-stone-light rounded-lg transition-colors inline-flex"
        >
        ...
        <div className="p-6 bg-red-50 border border-red-200 rounded-lg flex items-start gap-3">
          ...
          <div>
            <p className="text-red-900 text-sm font-sans font-medium">Failed to load blog post</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="font-serif text-3xl text-dark">Edit Blog Post</h1>
          <p className="text-warm font-sans">Update your blog post content</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white border border-stone-light rounded-lg p-6">
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <span className="font-sans text-sm text-dark">Published</span>
          </label>
          <p className="text-xs text-warm-light font-sans mt-2">
            {formData.isPublished
              ? 'This post is currently published and visible on your blog.'
